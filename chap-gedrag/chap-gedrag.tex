...
\begin{algorithm}
	...
	\KwOut{Een afbeelding van bericht naar string. De string stelt de voorwaarde voor waaronder naar een bericht wordt gesprongen bij de transitie naar een fragment.}
...
\begin{algorithm}
	...
	\KwOut{Een afbeelding van bericht naar string, zoals in algoritme \ref{alg:transition-to-frag}}
	\textit{uitvoer $\leftarrow \emptyset$}; \\
	\ForEach{\textit{kind} $\in$ \textit{kinderen}}{
	\If{kind is eerste kind $\land$ slaEersteOver}{
...
\begin{algorithm}
	...
	\textit{bundelVoorwaarde $\leftarrow$ voorwaarde van fragment}; \\
	\ForEach{container $\in$ containers}{
	\eIf{container is een fragment}{
	\textit{transities $\leftarrow$ bepaalTransitieInOnvolledig(container, $\epsilon$, uitgesloten)}; \\
...
Nu bereiken we stap 18 in algoritme \ref{alg:processCombinedFragment}. Deze stap houdt in dat we algoritme \ref{alg:calcExitForMessages} oproepen met \fragname{loop1} als argument. Dit algoritme wordt recursief opgeroepen voor alle kinderen.

Algoritme \ref{alg:determineFinalMessage} vindt eerst \fragmessage{message9} als laatste bericht van \fragname{loop1}. \fragmessage{message10} is geen deel van een fragment, dus noteert algoritme \ref{alg:exitToOutside} dat de uitvoering van \fragmessage{message9} springt naar \fragmessage{10} als ``\fragcond{loop1}'' niet meer geldt.

Vervolgens vindt algoritme \ref{alg:determineFinalMessage} \fragmessage{message4} en \fragmessage{message5} als laatste berichten van respectievelijk \fragname{alt1a} en \fragname{alt1b}. \fragmessage{message6} is deel van de ouder van \fragname{alt1}, namelijk \fragname{loop1}, dus wordt algoritme \ref{alg:transition-to-frag} opgeroepen met \fragname{loop2} als argument. Op deze manier wordt gevonden dat de uitvoering van \fragmessage{message4} naar \fragmessage{message6} springt als ``\fragcond{loop2} $\land$ \fragcond{loop3} $\land$ \fragcond{alt2a}'' geldt, naar \fragmessage{message7} als ``\fragcond{loop2} $\land$ \fragcond{loop3} $\land$ \fragcond{alt2b}'' geldt en naar \fragmessage{message8} als ``\fragcond{loop2} $\land \lnot$ \fragcond{loop3} $\land$ \fragcond{loop4}'' geldt. Het kan ook zijn dat de uitvoering van \fragmessage{message4} rechtstreeks springt naar \fragmessage{message9} omdat ``$\lnot$\fragcond{loop2}'' geldt. Diezelfde mogelijke sprongen gelden ook voor \fragmessage{message5}.

Algoritme \ref{alg:determineFinalMessage} duidt \fragmessage{message6} en \fragmessage{message7} aan als laatste berichten van respectievelijk \fragname{alt2a} en \fragname{alt2b}. \fragmessage{message8} is deel van de ouder van \fragname{alt2}, namelijk \fragname{loop2}, dus springt de uitvoering van \fragmessage{message6} en \fragmessage{message7} naar \fragmessage{message8} als ``$\lnot$\fragcond{loop3} $\land$ \fragcond{loop4}'' geldt. Algoritme \ref{alg:calcExitForMessages} zet nu een stap naar boven in de boom naar \fragname{loop1}. \fragmessage{message9} is daar een deel van, dus springt de uitvoering van \fragmessage{message6} en \fragmessage{message7} naar \fragmessage{message9} als ``$\lnot$\fragcond{loop2} $\land \lnot$ \fragcond{loop3} $\land \lnot$ \fragcond{loop4}'' geldt.

Vervolgens vindt algoritme \ref{alg:determineFinalMessage} \fragmessage{message8} als laatste bericht van \fragname{loop4}. Algoritme \ref{alg:calcExitForMessages} vindt geen bericht na \fragmessage{message8} dat deel is van de ouder van \fragname{loop4}, namelijk \fragname{loop2}, maar wel van de ouder van \fragname{loop2}, namelijk \fragname{loop1}. Dit is \fragmessage{message9}. De uitvoering springt dus van \fragmessage{message8} naar \fragmessage{message9} als ``$\lnot$ \fragcond{loop2} $\land \lnot$ \fragcond{loop4}'' geldt.
...
We bereiken stap 19 in algoritme \ref{alg:processCombinedFragment}. Algoritme \ref{alg:calculateLoopReentry} duidt \fragmessage{message6}, \fragmessage{message7}, \fragmessage{message8} en \fragmessage{message9} aan als mogelijke laatste berichten van de uitvoering van een lus. Vanaf \fragmessage{message6} en \fragmessage{message7} moet \'e\'en stap naar boven in de boom gezet worden voordat men een lusfragment tegenkomt. ``\fragcond{loop3}'' is dus de enige voorwaarde voor een heruitvoering van \fragcond{alt2} naast de \textit{if}- of \textit{else}-voorwaarde voor de respectievelijke delen. \fragmessage{message8} is zelf rechtstreeks deel van \fragname{loop4}, dus moet enkel ``\fragcond{loop4}'' gelden voor een heruitvoering. Als ``\fragcond{loop4}'' niet geldt maar wel ``\fragcond{loop2}'', landt de heruitvoering van \fragname{loop2} in \fragname{alt2} als ``\fragcond{loop3} geldt'' en ook ``\fragcond{alt2a}'' of ``\fragcond{alt2b}''. Als laatste moet om van \fragmessage{message9} naar \fragmessage{message2} te springen ``\fragcond{loop1}'' gelden. 
