...
\begin{align*}
	\transitionentry{\fragcond{voorwaarde}}{\fragmessage{$bericht\_a$}}
\end{align*}

Dit betekent dat de uitvoering kan springen naar $bericht_a$ onder \textit{\fragcond{voorwaarde}}.

\begin{align*}
	\transitionentry[\fragmessage{$bericht\_b$}]{\fragcond{voorwaarde}}{\fragmessage{$bericht\_a$}}
\end{align*}
...
In deze sectie gebruiken we figuur \ref{fig:seq-diagram-frag-ex} om een voorbeeld te geven van de vertaling van combined fragments volgens de algoritmes beschreven in secties \ref{sec:transition-to}, \ref{sec:transitions-out}, \ref{sec:loop-reentry} en \ref{sec:process-frag}.
...
Hoewel we de voorwaarde(s) van een fragment gebruiken om zowel te verwijzen naar het fragment als geheel als naar de voorwaarde zelf, schrijven we \fragname{loop1} om te verwijzen naar het fragment en \fragcond{loop1} om te verwijzen naar de voorwaarde. \fragname{alt1} verwijst naar het gehele alt-fragment en \fragname{alt1a} en \fragname{alt1b} verwijzen respectievelijk naar het \textit{if}-deel en het \textit{else}-deel.
...
\begin{align*}
	&\transitionentry{\fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2}}{\fragmessage{message6}} \\
	&\transitionentry{\fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2}}{\fragmessage{message7}}
\end{align*}
...
We keren terug naar het niveau van \fragname{loop1loop1}. Dit fragment heeft verder geen kinderen, dus eindigt deze oproep van algoritme \ref{alg:wrap-loops} hier.
...
\begin{align*}
	&\forall{t}[Time](C\_SDPointAt(Next(t), message2) \leftarrow (SDPointAt(t, message1) \\ &\lor SDPointAt(t, message9)) \land \fragcond{loop1}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message3) \leftarrow SDPointAt(t, message2) \\ &\land \fragcond{alt1a}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message5) \leftarrow SDPointAt(t, message2) \\ &\land \fragcond{alt1b}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message6) \leftarrow (SDPointAt(t, message6) \\ &\lor SDPointAt(t, message7) \land \fragcond{loop3} \land \fragcond{alt2a})). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message6) \leftarrow SDPointAt(t, message8) \\ &\land \lnot \fragcond{loop4} \land \fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2a}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message6) \leftarrow (SDPointAt(t, message4) \\ &\lor SDPointAt(t, message5)) \land \fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2a}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message7) \leftarrow (SDPointAt(t, message6) \\ &\lor SDPointAt(t, message7) \land \fragcond{loop3} \land \fragcond{alt2b})). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message7) \leftarrow SDPointAt(t, message8) \\ &\land \lnot \fragcond{loop4} \land \fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2b}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message7) \leftarrow (SDPointAt(t, message4) \\ &\lor SDPointAt(t, message5)) \land \fragcond{loop2} \land \fragcond{loop3} \land \fragcond{alt2b}).
\end{align*}

\begin{align*}
	&\forall{t}[Time](C\_SDPointAt(Next(t), message8) \leftarrow (SDPointAt(t, message6) \\ &\lor SDPointAt(t, message7)) \land \lnot \fragcond{loop3} \land \fragcond{loop4}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message8) \leftarrow (SDPointAt(t, message4) \\ &\lor SDPointAt(t, message5)) \land \fragcond{loop2} \land \lnot \fragcond{loop3} \\ &\land \fragcond{loop4}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message8) \leftarrow SDPointAt(t, message8) \\ &\land \fragcond{loop4}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message9) \leftarrow (SDPointAt(t, message6) \\ &\lor SDPointAt(t, message7)) \land \lnot \fragcond{loop3} \land \lnot \fragcond{loop4} \\ &\land \lnot \fragcond{loop2}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message9) \leftarrow SDPointAt(t, message8) \\ &\land \lnot \fragcond{loop4} \land \lnot \fragcond{loop2}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message9) \leftarrow (SDPointAt(t, message4) \\ &\lor SDPointAt(t, message5)) \land \lnot \fragcond{loop2}). \\
	&\forall{t}[Time](C\_SDPointAt(Next(t), message10) \leftarrow (SDPointAt(t, message1) \\ &\lor SDPointAt(t, message9)) \land \lnot \fragcond{loop1}).
\end{align*}
