...
Merk op in stap 1 dat \textit{laatsteBericht} later wordt gemapt naar een mapping van berichten naar de voorwaarde waaronder naar die berichten wordt gesprongen vanaf \textit{laatsteBericht}.
...
De recursieve oproepen van algoritme \ref{alg:transition-to-frag} zorgen ervoor dat alle fragmenten worden bekeken. Verder verzekeren ze ook dat de voorwaarde voor een ouderfragment wordt doorgegeven aan alle geneste fragmenten die zelf een fragment hebben als eerste container. Algoritme \ref{alg:wrap-loops} zorgt ervoor dat $\lnot$\fragcond{loop3} wordt opgenomen in de voorwaarde voor de uitvoering van \fragname{loop4}. De uitkomst van de uitvoering van algoritme \ref{alg:transition-to-frag} op \fragname{loop1} is:
...
Nu bereiken we stap 18 in algoritme \ref{alg:processCombinedFragment}. Deze stap houdt in dat we algoritme \ref{alg:calcExitForMessages} oproepen met \fragname{loop1} als argument. Dit algoritme wordt recursief opgeroepen op alle kinderen.
...
Vervolgens vindt algoritme \ref{alg:determineFinalMessage} \fragmessage{message4} en \fragmessage{message5} als laatste berichten van respectievelijk \fragname{alt1a} en \fragname{alt1b}. \fragmessage{message6} is deel van de ouder van \fragname{alt1}, namelijk \fragname{loop1}, dus wordt algoritme \ref{alg:transition-to-frag} opgeroepen op \fragname{loop2}. Op deze manier wordt gevonden dat de uitvoering van \fragmessage{message4} naar \fragmessage{message6} springt als \fragcond{loop2} $\land$ \fragcond{loop3} $\land$ \fragcond{alt2a} geldt, naar \fragmessage{message7} als \fragcond{loop2} $\land$ \fragcond{loop3} $\land$ \fragcond{alt2b} geldt en naar \fragmessage{message8} als \fragcond{loop2} $\land \lnot$ \fragcond{loop3} $\land$ \fragcond{loop4} geldt. Het kan ook zijn dat de uitvoering van \fragmessage{message4} rechtstreeks springt naar \fragmessage{message9} omdat $\lnot$\fragcond{loop2}. Diezelfde mogelijke sprongen gelden ook voor \fragmessage{message5}.
...
Hier stopt de uitvoering van algoritme \ref{alg:calcExitForMessages}. Samengevat was de uitvoer: 
