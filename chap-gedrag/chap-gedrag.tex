...
\begin{algorithm}
	\KwIn{\textit{fragment : combined fragment; uitvoer : verzameling van bericht $\rightarrow$ verzameling van \{bericht $\rightarrow$ string\}} \\
	De verwachting is dat \textit{uitvoer} leeg is bij de initi\"ele oproep}
	\KwOut{\textit{Verzameling van bericht $\rightarrow$ \{bericht $\rightarrow$ string\}}}
	\textit{aggregateVoorwaarde $\leftarrow \epsilon$} \\
	\textit{laatsteBericht $\leftarrow$ laatste bericht van fragment}; \\
	\If{laatsteBericht is rechtstreeks deel van fragment}{
	\textit{mapPaar $\leftarrow$ (laatsteBericht $\rightarrow \emptyset$)}; \tcc{laatsteBericht wordt later gemapt naar een mapping van berichten naar de voorwaarde waaronder naar die berichten wordt gesprongen vanaf laatsteBericht} 
	...
	\textit{als fragment een lusfragment is, voeg negatie van lusvoorwaarde toe aan aggregateVoorwaarde}; \\
	\textit{ouderFragment $\leftarrow$ ouder van fragment}; \\
	\If{bericht na laatsteBericht is deel van ouderFragment}{
	\textit{berichtNa $\leftarrow$ na laatsteBericht}; \\
	\textit{kind $\leftarrow$ voorouder van fragment waar berichtNa rechtstreeks deel van is dat ouderFragment als ouder heeft}; \\
	\textit{kinderenNa $\leftarrow$ \{kind\} $\cup$ alle kinderen van ouderFragment die na kind komen}; \\
	...
	\textit{transitiesIn $\leftarrow$ transitiesIn $\cup$ bepaalTransitiesIn(kindFragment, ``'')};}
	...
	\textit{uitvoer $\leftarrow$ uitvoer + (laatsteBericht $\rightarrow$ mapPaar);} \\
	\textit{ga naar stap 30 als het laatste lid van kinderenNa geen lus is, anders naar stap 23}}
	\eIf{ouderFragment heeft een ouder}{
	\textit{fragment $\leftarrow$ ouderFragment;} \\
	\textit{ga naar stap 5}}{
	\textit{transitieNaarBuiten(ouderFragment, mapPaar, aggregateVoorwaarde)}; \\
	\textit{uitvoer $\leftarrow$ uitvoer + (laatsteBericht $\rightarrow$ mapPaar)}; \\
	\textit{ga naar stap 30}}
	}
	}
	\ForEach{kind $\in$ kinderen van fragment}{
...
\begin{algorithm}
	...
	\textit{transities $\leftarrow$ bepaalTransitieInOnvolledig(fragment, $epsilon$)}; \\
	\textit{concateneer alle sprongvoorwaarden in transities met aggregateVoorwaarde}; \\
...
De mogelijke laatste berichten van een fragment zijn deze na ze uitgevoerd zijn ervoor zorgen dat het uitvoerpad het fragment verlaat, bijvoorbeeld de laatste berichten van de \textit{if}- en \textit{else}-gedeeltes van een alt-fragment of een bericht dat voorafgaat aan een lusfragment.

Vanaf zulk een laatst bericht stappen we naar boven in de boom en berekenen we elke keer als we een lusfragment tegenkomen de ingangspunten van dat fragment. Naar die ingangspunten wordt dan gesprongen indien de voorwaarde voor de lus niet meer geldt.
...
\begin{algorithm}
	...
	\textit{berichtVoor $\leftarrow$ het bericht dat vlak v\'o\'or fragment komt}; \\
	\ForEach{bericht---string-paar $\in$ transitieIn}{
	\textit{uitvoer $\leftarrow$ uitvoer + \{bericht $\rightarrow$ \{berichtVoor $\rightarrow$ string\}\}}}
	\textit{bepaalVerlaatpunten(fragment, uitvoer)}; \\
	\textit{verzekerLusHeruitvoering(fragment, uitvoer)};}
