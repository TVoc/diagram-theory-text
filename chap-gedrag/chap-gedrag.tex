...
Het doel is om te bepalen welke berichten dienen als punten waar het uitvoerpad een combined fragment verlaat (verder een 'verlaatpunt'), en naar welke berichten de uitvoer springt onder welke voorwaarden. Daarom is het resultaat van deze procedure een mapping van verlaatpunt naar een mapping van berichten en onder welke voorwaarde naar die berichten wordt gesprongen. We geven een overzicht van de procedure in algoritme \ref{alg:calcExitForMessages}.
...
\begin{algorithm}
	...
	Dit is een mapping van verlaatpunten naar een mapping van berichten waarnaar gesprongen wordt vanaf het verlaatpunt naar een string die specificeert onder welke voorwaarde naar dat bericht wordt gesprongen.}
	...
	\textit{transitieNaarBuiten(fragment, mapPaar)}; zie algoritme \ref{alg:exitToOutside} \\
	\textit{uitvoer $\leftarrow$ uitvoer + (laatsteBericht $\rightarrow$ mapPaar)};}{
	\If{er is een bericht meteen na fragment}{
	\textit{berichtNa $\leftarrow$ bericht meteen na fragment}; \\
	\textit{kind $\leftarrow$ voorouder van fragment waar berichtNa deel van is dat fragment als ouder heeft}; \\
	\textit{kinderenNa $\leftarrow$ alle kinderen van fragment die na kind komen}; \\
	\textit{als het eerste lid van kinderenNa geen lusfragment is, hou enkel dat eerste lid over; anders, hou eerste lid over en alle lusfragmenten die meteen volgen op het eerste lid} \\
	...
	\textit{als fragment een lusfragment is, concateneer alle sprongvoorwaarden in transitiesIn met de voorwaarde voor fragment} \\
	\textit{voeg alle elementen van transitiesIn toe aan mapPaar} \\
	\textit{uitvoer $\leftarrow$ uitvoer + (laatsteBericht $\rightarrow$ mapPaar);} \\
	\textit{ga naar stap 26}}
	\eIf{fragment heeft een ouder}{
	\textit{begin terug vanaf stap 4 met de ouder van fragment}}{
	\textit{transitieNaarBuiten(fragment, mapPaar, transitieVoorwaarde)}; \tcc{als fragment een alt-fragment is, is transitieVoorwaarde leeg; anders is transitieVoorwaarde de voorwaarde voor de lus}
	\textit{uitvoer $\leftarrow$ uitvoer + (laatsteBericht $\rightarrow$ mapPaar)}; \\
	\textit{ga naar stap 26}}
	}
	}
...
\begin{algorithm}
	...
	\textit{lussen $\leftarrow$ alle lussen die meteen volgen op fragment}; \\
	\textit{aggregateVoorwaarde $\leftarrow \epsilon$}; \\
	\ForEach{lus $\in$ lussen}{
	\textit{transitiesIn $\leftarrow$ bepaalTransitieIn(fragment, aggregateVoorwaarde)}; \\
	...
	\textit{aggregateVoorwaarde $\leftarrow$ aggregateVoorwaarde + ``$\land \lnot$('' + voorwaarde voor lus + ``)''}}
	\textbf{return};}
